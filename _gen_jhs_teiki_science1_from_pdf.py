# -*- coding: utf-8 -*-
"""
定期テスト対策ワーク 中1理科 — PDF をビューア用 PNG にレンダリングする。

既定ソース: D:\\Files\\(2025)定期テスト対策ワーク\\中1理科 内の最新 .pdf
出力: images/jhs_teiki_science1/page_0001.png …

  pip install pymupdf pillow

*本文.pdf を優先（解答解説 PDF は使わない）。表紙・サムネ・アイコンは _sync_jhs_teiki_covers_and_icons.py を実行。
完了後、js/jhs_data.js の jhs_teiki_science1.totalPages をページ数に合わせてください。
"""
...
DEFAULT_SRC_DIR = Path(r"D:\Files\(2025)定期テスト対策ワーク\中1理科")
OUT_DIR = ROOT / "images" / "jhs_teiki_science1"
ZOOM = 2.0
...
def main() -> int:
    ...
    print(f"totalPages を {n} に設定: js/jhs_data.js の jhs_teiki_science1")
    return 0
